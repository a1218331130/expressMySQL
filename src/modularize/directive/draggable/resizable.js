...
class Resizable extends Events {
  ...
  init(options) {
     ...
     * @member {Object}
     */
    this.options = Object.assign({}, defaultOptions, options === false ? { disabled: true } : options || {})
  ...
  handleMouseLeave() {
    this.isMouseEnter = !this.isResizing && this.setCursor(null)
    return false
  }
  ...
  resize({ clientX, clientY }) {
    ...
    if (data.dir.includes('e')) {
      ...
      width = Math.min(Math.max(width, o.minWidth), o.maxWidth)
      data.width = width
    }
    if (data.dir.includes('s')) {
      let height = data.startHeight + clientY - data.startY
      height = Math.min(Math.max(height, o.minHeight), o.maxHeight)
      data.height = height
    }
 ...
 *  <div v-resizable="{}"></div>
 *
 *  import resizable from '@modularize/util/directives/dresizable'
 *  export default {
 *    directives: {
 *      resizable
 *    }
