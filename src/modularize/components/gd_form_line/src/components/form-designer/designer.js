...
export function createDesigner(vueInstance) {
  ...
  return {
    ...
    insertTableRow(widget, insertPos, cloneRowIdx, isSplit = false) {
      ...
      let colNo = 0
      while (rowIdx < widget.rows.length - 1 && colNo < widget.rows[0].cols.length) {
        //越界判断
      ...
      newRow.cols = [
        {
          ...
          internal: true,
          merged: false,
          options: { name: 'table-cell-' + generateId(), cellWidth: '', cellHeight: '', colspan: 0, rowspan: 0, ...newRow.cols[0].options },
          showInput: false,
          type: 'table-cell',
          widgetList: []
        }
    ...
    splitTabRowCol(widget, rows, cols, splitRow, splitCol) {
      this.splitMethod(widget, rows + 1, rows, cols)
    },
    insertTableCol(widget, insertPos) {
      ...
      let rowNo = 0
      while (colIdx < widget.rows[0].cols.length - 1 && rowNo < widget.rows.length) {
        //越界判断
    ...
    copyNewContainerWidget(originObj, row = 2, col = 2) {
      let newCon = deepClone(originObj)
      newCon.id = newCon.type.replace(/-/g, '') + generateId()
      newCon.options.name = newCon.id
      console.log(originObj, 'originorigin')
      console.log(newCon, 'newConnewCon')
      if (newCon.type === 'grid') {
    ...
    addContainerByDbClick(container, row = 2, col = 2) {
      let newCon = this.copyNewContainerWidget(container, row, col)
    ...
    saveFormContentToStorage() {
      localStorage.setItem('widget__list__backup', JSON.stringify(this.widgetList))
      localStorage.setItem('form__config__backup', JSON.stringify(this.formConfig))
    },
