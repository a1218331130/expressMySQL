<template>
  <span>
    <i v-if="isIconFont || isElIcon" :class="isElIcon ? iconName : `iconfont-${iconSuffix} ${cssPrefixText}${iconName}`" :style="IconClass" />
    <!-- 处理导航栏菜单配置iconfont -->
    ...
    <svg v-else :class="svgClass" :style="iconStyle" aria-hidden="true" v-on="$listeners">
      <use :xlink:href="!isLocalSvg ? `#${cssPrefixText}${iconName}` : `#${iconName}`" />
    </svg>
  </span>
</template>

<script>
import svgNameList from '@modularize/icons'
export default {
  name: 'SvgIcon',
  props: {
    setIconClass: {
  ...
  computed: {
    IconClass() {
      if (this.iconClass.includes('[')) {
        return { color: this.iconClass.match(/\[(\S*)\]/)[1], ...this.setIconClass }
      } else {
        return this.setIconClass
      }
    },
    isIconFont() {
      return this.iconClass.includes('[') || this.iconClass.includes('iconfont')
    },
    iconName() {
      if (this.iconClass.includes('[')) {
